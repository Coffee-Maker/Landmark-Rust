...
pub fn trigger_card_behaviors(card_instance_id: CardInstanceId, trigger_owner: PlayerId, trigger_name: CardBehaviorTriggerWhenName, context: CardBehaviorTriggerContext, state: &mut GameState, communicator: &mut GameCommunicator) -> Result<()> {
    ...
    for behavior in &card.behaviors.clone() { // Todo: Is clone required here? I assume so. Ask Marc
        ...
        if successful_triggers.len() > 0 {
            for action in &behavior.actions {
                process_behavior_action(&action, &context, state, communicator);
            }
        }
    }
...
fn process_behavior_action(action: &CardBehaviorAction, context: &CardBehaviorTriggerContext, state: &mut GameState, communicator: &mut GameCommunicator) {
    match action {
